...
export const handler = async (context, argv) => {
  ...
  if (argv.AuthenticationToken) {
    ...
    if (!authToken) {
      console.log(
        chalk.red("✘ Authentication token is required to sign into the Maana CLI")
      );
      return;
    }
  ...
  try {
    ...
    maanaOptions.auth = Buffer.from(JSON.stringify(authInfo)).toString(
      "base64"
    );
    ...
    if (e.response && e.response.statusCode) {
      console.log(chalk.red(`✘ Failed to sign in with status ${e.response.statusCode}`));
      if (e.response.body) {
